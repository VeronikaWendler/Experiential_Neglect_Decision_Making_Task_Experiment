function ExpSymbNeglect_final2(participant)
...
trainingInstr = ['TRAINING: Instructions for the first training task:'...
    ...
    '\n\n\n Importantly, your choices in the Training half will NOT be considered.'...
    ' Only the points won during the Experimental half will count towards your payoff.'...
    '\n\n\n Please press the space bar to continue.'];
...
allImagesTEST = {Circle, Cursor, Folder, AI, At, Cloud, Hazel, Star};
allImageNamesTEST = {'Circle', 'Cursor', 'Folder', 'AI', 'At', 'Cloud', 'Hazel', 'Star'};
...
for block = 1:3
    ...
    %This task may seem challenging, but the process is as follows: We generate arrays consisting of ones and minus ones 
    %based on the specified probability array and its complement, denoted as P'. Subsequently, the 'horzcat' function is 
    %employed to horizontally concatenate these arrays. The randomization of the selection procedure is then accomplished 
    %through the use of the 'shuffle' function. We evaluate the count of trials resulting in a minus one outcome and the 
    %count of trials resulting in a one outcome
...
line21 = 'TRAINING: Instructions for the second training task:';
line22 = '\n\n  In the second task there will be two kinds of options.';
line23 = '\n\n\n  The first kind of option is the symbols you have already encountered during the previous task.';
line24 = '\n\n Note: The symbols maintain the same odds of winning/losing a point as in the first task.';
...
line33 = 'TRAINING: Instructions for the second training task:';
line34= '\n\n In each round, you have to choose between one of two symbols displayed on either side of the screen.';
line35 = '\n\n\n  You can select the left symbol by presseing LEFT arrow and the right symbol by pressing RIGHT arrow.';
line36 = '\n\n\n  Please note that in this task, no outcome will be displayed, such that after a choice, the next pair of options will be shown without an intermediate step.';
line38 = '\n\n\n  At the end of the task, you will be shown the final payoff in terms of cumulative points and monetary bonus';
...
line49 = '\n\n\n   You can click on the slider with the mouse cursor and then click on the confidence rating scale that feels appropriate';
...
allImagesEXP = {Fire, Menu, Magnet, Intersection, Mark, Cycle};
allImageNamesEXP = {'Fire', 'Menu', 'Magnet', 'Intersection', 'Mark', 'Cycle'};
...
line52 = '\n\n 1) Training Phase: You will be introduced to symbols and their probabilities.';
line53 = '\n\n\n 2) Testing Phase of the experiment: You will be asked to choose between these icons and piecharts.';
...
line59 = '\n\n On each trial, you have to choose between one of two symbols displayed on either side of the screen.';
line60 = '\n\n\n To select the left symbol, use the LEFT arrow; to select the right symbol, use the RIGHT arrow.';
...
line89 = '\n\n This is the actual game, every point here will count towards the final payoff. Ready?';
line90 = '\n\n\n Please press the space bar to continue.';
...
for trialEXP2 = 1:totalTrialsEXP2
    ...
    if objLocation
       ...
       pieRandVars = generatePieChartPTB(piechartProbEXP2, piechartTypeEXP2, ptbwin,CenterRect(ScaleRect(rightPositionEXP2,0.85,0.85),rightPositionEXP2));
       Screen('FrameRect', window, black, rightPositionEXP2, 3);
       ...
       pieRandVars = generatePieChartPTB(piechartProbEXP2, piechartTypeEXP2, ptbwin, CenterRect(ScaleRect(leftPositionEXP2,0.85,0.85),leftPositionEXP2));
       Screen('FrameRect', window, black, leftPositionEXP2, 3);
    ...
    if objLocation
        Screen('DrawTexture', window, textureEXP2, [], leftPositionEXP2);
        generatePieChartPTB(piechartProbEXP2, piechartTypeEXP2, ptbwin, CenterRect(ScaleRect(rightPositionEXP2,0.85,0.85),rightPositionEXP2), pieRandVars);
        Screen('FrameRect', window, black, rightPositionEXP2, 3);
    else
        Screen('DrawTexture', window, textureEXP2, [], rightPositionEXP2);
        generatePieChartPTB(piechartProbEXP2, piechartTypeEXP2, ptbwin, CenterRect(ScaleRect(leftPositionEXP2,0.85,0.85),leftPositionEXP2), pieRandVars);
        Screen('FrameRect', window, black, leftPositionEXP2, 3);
    end
    ...
    if ~mod(trialEXP2,40) && trialEXP2 ~= totalTrialsEXP2
        Screen('DrawText',window,'Please take a short break',60,60,black);
        Screen('DrawText',window,'Please wait 2 seconds and press space to begin new block.',60,150,black);
        currScoreMessage = ['So far you have won ', num2str(scoreEXP2), ' points. \n Press space to continue to the final task'];
        DrawFormattedText(window, currScoreMessage, 'center', 'center', black);
        Screen('Flip',window);
        KbStrokeWait;
    end
end
...
% Display the final score
finalScoreMessage = ['Overall, you have won ', num2str(scoreEXP2), ' points! Congrats! \n Press space to continue to the final task'];
DrawFormattedText(window, finalScoreMessage, 'center', 'center', black);
Screen('Flip', window);
KbStrokeWait;
...
allpienames = zeros(1,4*7);
for pietypes = 1:4
    pienames = pietypes*10+(1:7);
    allpienames((pietypes-1)*7+1:pietypes*7) = pienames;
end
numtrialspericon = 3;
sliderimagenames = Shuffle(repelem([1:6, allpienames],numtrialspericon));
numTrialsSliderEXP = length(sliderimagenames);
...
imgloc = CenterRectOnPoint([0 0 imageSize], xCenter, yCenter - imageSize(2)/8);

for trial = 1:numTrialsSliderEXP
    curricon = sliderimagenames(trial);
    if curricon < 10                                            
        selectedImageNameSliderEXP{trial} = selectedImagesFromPartsEXP2{curricon};
        selectedImageSliderEXP = allImagesEXP{strcmp(allImageNamesEXP, selectedImageNameSliderEXP{trial})};                                                      
        imgTex = Screen('MakeTexture', window, selectedImageSliderEXP);                         
    else
        probabilityPie = allPieChartProbabilities(mod(curricon,10));
        piechartType = floor (curricon/10);
        pieRandVars = generatePieChartPTB(probabilityPie, piechartType, ptbwin, CenterRect(ScaleRect(imgloc,0.85,0.85),imgloc));
        Screen('FrameRect', window, black, imgloc, 3);
        selectedImageNameSliderEXP{trial} = ['Pie' num2str(curricon)];
    end
    ...
    while true
        if curricon < 10
            Screen('DrawTexture', window, imgTex, [], imgloc);
        else
            generatePieChartPTB(probabilityPie, piechartType, ptbwin, CenterRect(ScaleRect(imgloc,0.85,0.85),imgloc), pieRandVars);
            Screen('FrameRect', window, black, imgloc, 3);
        end
        [mx, my, buttons] = GetMouse(window);
        if buttons(1)
